...
document.getElementById('d2-mary').addEventListener('click',function(){var m=document.getElementById('d2-mary'),e=document.getElementById('d2-elizabeth');if(!d2Done){m.classList.add('walk');setTimeout(function(){d2Done=true;m.classList.add('arrive');e.classList.add('show');dayDone[1]=true;pk(lang,'j2Success');},1600);}});
...
document.getElementById('d3-bed').addEventListener('click',function(){if(d3Done)return;d3Done=true;document.getElementById('d3-dream').classList.add('show');dayDone[2]=true;pk(lang,'j3Success');});
...
(function(){var scene=document.getElementById('d4-scene');if(!scene)return;var dragging=false;function getPct(cx){var r=scene.getBoundingClientRect();return Math.max(0,Math.min(1,(cx-r.left)/r.width));}function onMove(cx){var pct=getPct(cx);var left=6+pct*76;var d=document.getElementById('d4-donkey');if(d)d.style.left=Math.min(82,Math.max(6,left))+'%';var f=Math.max(0,(left-6)/76*100);document.getElementById('d4-progress-fill').style.width=f+'%';if(f>85&&!d4Done){d4Done=true;pk(lang,'j4Success');}}scene.addEventListener('touchstart',function(e){dragging=true;},{passive:true});scene.addEventListener('touchmove',function(e){if(dragging)onMove(e.touches[0].clientX);},{passive:true});scene.addEventListener('touchend',function(){dragging=false;},{passive:true});scene.addEventListener('mousedown',function(){dragging=true;});scene.addEventListener('mousemove',function(e){if(dragging)onMove(e.clientX);});scene.addEventListener('mouseup',function(){dragging=false;});})();
...
document.getElementById('d5-star').addEventListener('click',function(){if(d5Done)return;d5Done=true;var s=document.getElementById('d5-star'),j=document.getElementById('d5-jesus'),g=document.getElementById('d5-glow');s.classList.add('done');s.style.top='18%';setTimeout(function(){if(g)g.classList.add('show');},400);setTimeout(function(){if(j)j.classList.add('show');dayDone[4]=true;pk(lang,'j5Success');},800);});
...
document.getElementById('d6-sheep').addEventListener('click',function(){d6Clicks++;if(d6Clicks===1){document.getElementById('d6-sheep').style.display='none';document.getElementById('d6-shepherd').classList.add('show');}else if(d6Clicks===2){document.getElementById('d6-angel').classList.add('show');dayDone[5]=true;pk(lang,'j6Success');} });
...
document.getElementById('d7-star').addEventListener('click',function(){d7Clicks++;var s=document.getElementById('d7-star'),m=document.getElementById('d7-magi'),g=document.getElementById('d7-gift');if(d7Clicks===1){s.classList.add('moved');setTimeout(function(){if(m)m.classList.add('walk');},400);}else if(d7Clicks>=2){if(g)g.classList.add('show');dayDone[6]=true;pk(lang,'j7Success');} });
